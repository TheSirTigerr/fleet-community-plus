...
func TestSQLStoreOIDCSettings(t *testing.T) {
	...
	mock.ExpectQuery("SELECT enabled, issuer_url, client_id, client_secret, scopes, idp_name").
		WillReturnRows(sqlmock.NewRows([]string{
			"enabled", "issuer_url", "client_id", "client_secret", "scopes", "idp_name",
		}).AddRow(
			true,
			"https://idp.example",
			"fleet-client",
	...
	if !settings.Enabled || settings.ClientID != "fleet-client" || settings.ClientSecret != "secret" || len(settings.Scopes) != 3 {
...
func TestSQLStoreOIDCSettingsDefaultsWhenRowMissing(t *testing.T) {
	...
	if settings.Enabled || len(settings.Scopes) != 2 || settings.Scopes[0] != "openid" || settings.Scopes[1] != "email" {
